...
pub fn build_rotation_plan(bridges: &[Value], censorship_level: u8, max_entries: usize) -> Value {
    ...
    let cap = if max_entries == 0 { usize::MAX } else { max_entries };
...
mod tests {
    ...
    fn plans_alternate_transports_for_dpi_wave_resilience() {
        let bridges = vec![
            bridge("obfs4 203.0.113.1:443 AAAA cert=AAAA iat-mode=0", "obfs4", 0.99),
            bridge("obfs4 203.0.113.2:443 BBBB cert=BBBB iat-mode=0", "obfs4", 0.98),
    ...
    fn escalated_censorship_promotes_fronting_transports() {
        let bridges = vec![
            bridge("obfs4 203.0.113.9:443 DDDD cert=DDDD iat-mode=0", "obfs4", 0.9),
    ...
    fn deterministic_for_identical_input() {
        let bridges = vec![
            bridge("obfs4 203.0.113.1:443 AAAA cert=AAAA iat-mode=0", "obfs4", 0.5),
    ...
    fn skips_records_without_lines_and_reports_histogram() {
        let bridges = vec![
            ...
            bridge("meek_lite 192.0.2.20:443 ZZZZ url=https://meek.example/", "meek_lite", 0.7),
        ];
    ...
    fn write_rotation_outputs_persists_plan_and_export() {
        let dir = std::env::temp_dir().join(format!(
            "iran-smart-rotation-test-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let bridges = vec![
            bridge("obfs4 203.0.113.1:443 AAAA cert=AAAA iat-mode=0", "obfs4", 0.8),
