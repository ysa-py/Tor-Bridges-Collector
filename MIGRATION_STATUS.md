...
> **Automated parity run note (Session 12, 2026-07-13):** An autonomous parity verification was executed against the repository in this sandbox. The run produced environment-level failures (Python test harness unavailable: `pytest` missing; Rust build toolchain missing MSVC linker `link.exe`), preventing a full byte-for-byte parity pass. Artifacts from the run were recorded: `python_test_output.txt`, `rust_test_output.txt`, `parity_hashes.txt`, `parity_run_metadata.txt`. See the Session 12 entry in `CHANGELOG.md` for details.
