// Parity tests for `src/ai_anti_dpi_iran.rs` vs `ai_anti_dpi_iran.py`.
//
// Everything here is pure (no network I/O), so every test is a
// straightforward differential comparison against real Python via
// subprocess, the same pattern as `iran_dpi_shaper_parity.rs`. The one
// exception is `get_tls_randomization`'s hourly rotation, which reads the
// real wall clock on both sides — tested against the deterministic,
// time-injected `get_tls_randomization_at` instead (already covered by
// `src/ai_anti_dpi_iran.rs`'s own internal unit tests), not differentially.
...
const ANALYZE_THREATS_SCRIPT: &str = r##"
import json, sys
...
const EVASION_STRATEGY_SCRIPT: &str = r##"
import json, sys
from ai_anti_dpi_iran import IranAntiDPI
...
const SNI_EVASION_SCRIPT: &str = r##"
import json, sys
from ai_anti_dpi_iran import IranAntiDPI
...
const TRAFFIC_SHAPING_SCRIPT: &str = r##"
import json, sys
from ai_anti_dpi_iran import IranAntiDPI
...
const ANALYZE_ENTROPY_SCRIPT: &str = r##"
import json, sys
from ai_anti_dpi_iran import IranAntiDPI
...
const OPTIMIZE_BRIDGE_SCRIPT: &str = r##"
import json, sys
from ai_anti_dpi_iran import IranAntiDPI
